...
/*
 *   $Id: MBeanRegistry.java,v 1.13 2007/05/05 05:25:35 tcfujii Exp $
 *   @author: alexkrav
 *
 *   $Log: MBeanRegistry.java,v $
 *   Revision 1.13  2007/05/05 05:25:35  tcfujii
 ...
 *   Reviewed by: Kedar
 *   Affected modules: com.tongweb.admin-core/com.tongweb.admin;
 *   Tests: QLT/EE
 *
 *   Revision 1.7  2006/05/08 17:18:53  kravtch
 *   Bug #6423082 (request for com.tongweb.admin infrastructure to support the com.tongweb.admin.config changes without DAS running (offline))
 *   Added infrastructure for offline execution under Config Validator for:
 *      - dottednames set/get operation
 *      - Add/remove jvm-options
 *   Submitted by: kravtch
 *   Reviewed by: Kedar
 *   Affected modules: com.tongweb.admin-core/com.tongweb.admin; com.tongweb.admin/validator;
 *
 ...
 *   Patch added to MBeanRegistry: every time when new com.tongweb.admin.config element is addeded - com.tongweb.admin.config context of the root element will be propagated to its children
 *   Bug #6171788
 *   Reviewer: Sridatta
 *   Tests passed: QLT +EE
 *
 ...
 *   com.tongweb.admin/com.tongweb.admin.meta/AdminConfigEventListener: new configcontext listener's code
 *   This listener is for synchronization of ConfigBeans changes with both MBeans and dotted-name spaces.
 *   com.tongweb.admin/com.tongweb.admin.meta/MBeanRegistry: added methods (adoptConfigBeanDelete/Add) implementing beans ajustment
 *   com.tongweb.admin/com.tongweb.admin.config/BaseConfigMBean: calls from MBean's postRegister/unregister methods to dotted-name-manager is commented out.
 *
 *   Reviewer: Sridatta
 ...
 *   (1) com.tongweb.admin/com.tongweb.admin-core/com.tongweb.admin-cli/com.tongweb.admin-gui/appserv-core/assembly-tool: switch to new domain name "ias:" -> "com.sun.appserv"
 *   (2) com.tongweb.admin-core and com.tongweb.admin-cli: switch to "dashed" attribute names
 *   (3) com.tongweb.admin-core: support both "dashed"/"underscored" names for get/setAttributes
 *   (4) com.tongweb.admin-gui: hook for reverse converting attribute names (temporary hack);
 *
 ...
 *      -AdminContext expanded by two new methods getAdminMBeanResourcUrl() and getRuntimeMBeanResourcUrl() which used by MBeanRegistryFactory for initialization com.tongweb.admin and runtime registries. So, they are become pluggable.
 *      -AdminContext also notifies MBeanRegistryFactory during its construction. So, AdminContext become "visible" to com.tongweb.admin-core/com.tongweb.admin classes.
 ...
 *   - getDefaultAttributeValue() implemented for com.tongweb.admin.config MBeans;
 *   - merge Jsr77 and com.tongweb.admin.config activity in runtime mbeans;
 *
 ...
 *   com.tongweb.admin/dtds/runtime-mbeans-descriptors.xml - modified to represent new runtime mbeans;
 *
...
import com.tongweb.admin.BaseAdminMBean;
import com.tongweb.admin.meta.MBeanMetaException;
import com.tongweb.admin.meta.MBeanMetaHelper;
import com.tongweb.admin.meta.naming.MBeanNamingDescriptor;
import com.tongweb.admin.meta.naming.MBeanNamingException;
import com.tongweb.admin.meta.naming.MBeanNamingInfo;
import com.tongweb.admin.meta.naming.Name;
import com.tongweb.config.ConfigBean;
import com.tongweb.config.ConfigBeansFactory;
import com.tongweb.config.ConfigContext;
import com.tongweb.config.ConfigException;
import com.tongweb.config.api.ServerXPathHelper;
import com.tongweb.config.serverbeans.ServerTags;
import com.tongweb.logging.LogDomains;
import com.tongweb.util.FeatureAvailability;
...
//        MBeansNaming.initMBeanNaming();
    ...
    public MBeanRegistryEntry findMBeanRegistryEntry(ObjectName objectName)
    {
        ...
        {
            _sLogger.log(Level.FINEST, MSG_FINDREGISTRYENTRY_FAILED, e);
        }
        return null;
    }
    //**************************************************************************
    ...
    /** 
     * This method can be used to register all com.tongweb.admin.config mbeans that are
     ...
     *
     * @param ctx ConfigContext used for creating com.tongweb.admin.config mbeans
                                ...
                                throws MBeanMetaException
    {
        ...
        catch (ConfigException e)
        {
            ...
            throw new MBeanMetaException
                ("ConfigException in getting Domain bean from com.tongweb.admin.config context", e);
        }
...
//register here
    ...
    {
        try {
            return new ObjectName("com.tongweb:name=dotted-name-registry,type=dotted-name-support"); //CHANGE_ME
        }
        catch (Exception e)
        {
            e.printStackTrace();
    ...
    {
        _sLogger.log(Level.FINEST, "******regDottedName******* "+dottedName +" ObjectName="+objectName);
    ...
    {
        _sLogger.log(Level.FINEST, "******unregDottedName******* for ObjectName="+objectName);
        ...
        for(int i=0; i<arr.size(); i++)
        {
            ...
            if(objectName!=null)
            {
                ...
                catch (Throwable t)
                {
                    ...
                    _sLogger.finest("!!!!!!!!!!!!!! Can not unregister dotted name for MBean: "+objectName);
                }
...
//                        notifyUnregisterMBean(objectName, getConfigContext());
                ...
                {
                    //this is ok, some elements has no dotted name
                    _sLogger.finest("!!!!!!!!!!!!!! Can not unregister MBean: "+objectName);
                }
            }
        }
   }
   ...
   private void collectChildrenObjectNames(ConfigBean bean, String domainName, ArrayList arr)
   {
        ...
        {
            String xpath = bean.getAbsoluteXPath("");
            _sLogger.log(Level.FINEST, "Object name malformed for bean: "+ xpath, e);
        } 
   }
   ...
   public MBeanNamingInfo getNamingInfoForConfigBean(ConfigBean bean, String domainName)
    {
        ...
        {
            // it is ok. not all com.tongweb.admin.config beans have entries in registry
            return null;
        }
   }
   /* ****************************************************************************************************
