...
/*
 *   $Id: MBeanRegistryEntry.java,v 1.10 2007/07/02 17:49:53 msreddy Exp $
 *   @author: alexkrav
 *
 *   $Log: MBeanRegistryEntry.java,v $
 *   Revision 1.10  2007/07/02 17:49:53  msreddy
 *   bug#6509963, added lb-enabled attribute for dynamic reconfig flag which will indicate whether reconfig is needed, reviewed by Kedar, passed ee quicklook tests
 *
 ...
 *   First, ssl-client-com.tongweb.admin.config is created with ssl subelement in it then it added to condig context.
 *   + ManagedConfigBean swithed from enclosing associated MBean to MBeanInfo.
 *   Submitted by: kravtch
 *   Reviewed by: Sreeni
 *   Affected modules com.tongweb.admin/mbeans; com.tongweb.admin-core/com.tongweb.admin
 ...
 *      - throwing exception for com.tongweb.admin.config MBeans if error in creation of ConfigBean;
 ...
 *   1. ALL com.tongweb.admin.config mbeans are now covered by descriptors.xml
 ...
 *   5. MBeanRegistryFactory has now different methods to obtain com.tongweb.admin/runtime registries
...
import com.tongweb.admin.meta.MBeanMetaConstants;
...
import com.tongweb.admin.meta.naming.MBeansNaming;
...
public class MBeanRegistryEntry implements MBeanMetaConstants
{
    ...
    public MBeanRegistryEntry(ManagedBean mb, boolean bMergeWithMbean) throws MBeanNamingException,ClassNotFoundException
    {
        ...
        if("config".equals(group))
        {
            if(clazz==null)
            {
                clazz = "com.tongweb.admin.config.BaseConfigMBean";
                managedBean.setClassName(clazz);
            }
            else
            {
                if(clazz.indexOf('.')<0)
                    clazz = "com.tongweb.admin.config.mbeans."+clazz;
            }
        ...
        else if("runtime".equals(group))
        {
            if(clazz==null)
            {
                clazz = "com.tongweb.admin.runtime.BaseRuntimeMBean";
                managedBean.setClassName(clazz);
            }
            else
            {
                if(clazz.indexOf('.')<0)
                    clazz = "com.tongweb.admin.runtime.mbeans"+clazz;
                //com.tomgweb.admin.runtime.mbeans.AnyRuntimMBeans;
            }
                                   ...
                                   throws Exception
    {
        ...
        if("config".equals(group))
        {
            if(xpath!=null && xpath.length()>0)
            {
                ...
                if(configBean!=null)
                    ...
                    throw new MBeanMetaException("instantiateMBean(): com.tongweb.admin.config mbean does not exist for xpath - " + xpath );
            }
            return mbean;
        }
        if( "runtime".equals(group))
        {
            ...
            if(beanClassName==null || beanClassName.length()==0)
                beanClassName = "com.tongweb.management.model."+namingInfo.getType()+"Mdl";
