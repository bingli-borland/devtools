...
public class DealerToRouter {
    ...
    public static class Worker implements Runnable {
        ...
        public void run() {
            ...
            // 首先send消息
            socket.send(("Hello " + name).getBytes(), NOFLAGS);
            while (!finished) {
                byte[] data = socket.recv(NOFLAGS);
                System.out.println(name + "get content: " + new String(data));
    ...
    throws InterruptedException {
        ...
        byte[] bytes = socket.recv(NOFLAGS);
        if (bytes != null) {
            System.out.println("router content: " + new String(bytes));
        }
        bytes = socket.recv(NOFLAGS);
        if (bytes != null) {
            System.out.println("router content: " + new String(bytes));
        }
        ...
        for (int i = 0; i < 10; i += 1) {
            ...
            socket.send(("This is the workload." + new String(address)).getBytes(), NOFLAGS);
        }
